...
function login() {
    ...
    json('http://37.187.122.134:8069/web/session/authenticate', {
        'base_location': 'http://37.187.122.134:8069',
        'db': 'odoo_test',
        'login': 'admin',
        'password': 'CSC-9742794200'
        //'session_id': session_id
    }).done(function (data) {
...
function do_fetch() {
    ...
    json('http://37.187.122.134:8069/web/dataset/search_read', {
        'model': 'res.partner',
        'fields': ['name', 'city']
        //'session_id': session_id
    }).then(function (data) {
...
function get_list_po() {
	json('http://37.187.122.134:8069/web/dataset/search_read', {
        ...
        var o;
        for (o in data.records) {
            //str += "<li>" + data.records[o].name + " - " + data.records[o].state + "</li>";
			str += "<li>" + data.records[o].name + "</li>";
        }
        str += "</ul>";
        $('#fetch').html(str);
	}).fail(function (error) {
...
function get_list_po_using_kw() {
	
	json('http://37.187.122.134:8069/web/dataset/execute_kw', {
       "model" : "res.partner",
        "method" : "search_read"
        //"args" : [1],  // id of record
...
function update_po() {
	json('http://37.187.122.134:8069/web/dataset/write', {
        'model': 'res.partner',
		'domain': [['is_company', '=', true]],
		//'is_company':true,
        'fields': ['name']
...
$(document).ready(function () {
    ...
    //login().then(do_fetch);
	login().then(get_list_po_using_kw());
});
